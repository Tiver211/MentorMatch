...
class Offer_table(Base):
    ...
    mentor_id: UUID = Column(UUIDP(as_uuid=True), ForeignKey('departments.id', ondelete='CASCADE'), nullable=False)
    user_id: UUID = Column(UUIDP(as_uuid=True), ForeignKey('departments.id', ondelete='CASCADE'), nullable=False)
