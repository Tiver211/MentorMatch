...
def send_request(request: Request, db: Session = Depends(get_db), authorization: str = Header(...)):
    ...
    new_request = Mentors_requests_table(request_id=uuid4(), user_id=data["sub"], about=request.about, direction=request.directions, date=datetime.datetime.now(), status=None)
